...
void read (int q2BinIndx)
{
  ...
  float Fl_0b=0, P1_0b=0, P5p_0b=0;
  ...
  for (int i=0; i<=100; i++) {
    ...
    nfound=0;
    isDone = false;
    for (int cnt=0; cnt<=max_scan; cnt++) open(q2BinIndx, i, cnt);
    if (isDone) nDone++;
  ...
  cout<<"Converging "<<h_Flb[q2BinIndx-1]->GetEntries()<<" / "<<nDone<<endl;
  ...
  c[q2BinIndx-1]->SaveAs(Form("plots/Sys2_data_bin%i.pdf",q2BinIndx-1));
