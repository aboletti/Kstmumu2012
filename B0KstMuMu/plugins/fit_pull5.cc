...
void read (int q2BinIndx, int toyIndx, int genIndx=0)
{
  ...
  hprof_P1[q2BinIndx] = new TH1F(Form("hprofP1%i",q2BinIndx),Form("hprofP1%i",q2BinIndx),600,-1,1);
  hprof_P5[q2BinIndx] = new TH1F(Form("hprofP5%i",q2BinIndx),Form("hprofP5%i",q2BinIndx),900,-1.5,1.5);
  ...
  TH1F* tmplike = new TH1F("tmplike","",101,0,1.01);
  for (int i=1; i<601; i++) for (int j=1; j<901; j++) {
      ...
      float xP1 = hprof_P1[q2BinIndx]->GetXaxis()->GetBinCenter(i);
      float xP5 = hprof_P5[q2BinIndx]->GetXaxis()->GetBinCenter(j);
      int indx;
	...
	tmplike->AddBinContent(tmplike->FindBin(val),val);
	if (val>hprof_P1[q2BinIndx]->GetBinContent(i)) hprof_P1[q2BinIndx]->SetBinContent(i,val);
	if (val>hprof_P5[q2BinIndx]->GetBinContent(j)) hprof_P5[q2BinIndx]->SetBinContent(j,val);
	if (val > max_val) {
	  max_val = val;
	  max_P1 = xP1;
	  max_P5 = xP5;
  ...
  for (float limit = 1; prob < tot_prof*0.6827; limit-=0.001) {
    ...
    double newmax = 0;
    int imax;
    for (int i=1; i<601; i++) if (hprof_P1[q2BinIndx]->GetBinContent(i)>limit) {
	prob += hprof_P1[q2BinIndx]->GetBinContent(i);
	hboun_P1[q2BinIndx]->SetBinContent(i,1);
      }
    for (int i=1; i<901; i++) if (hprof_P5[q2BinIndx]->GetBinContent(i)>limit) {
	prob += hprof_P5[q2BinIndx]->GetBinContent(i);
	hboun_P5[q2BinIndx]->SetBinContent(i,1);
      }
  }
  ...
  cout<<lastval<<" "<<tmplike->Integral(tmplike->FindBin(lastval),102)/tmplike->Integral(0,102)<<endl;
...
void fit_pull5 (int toyIndx=0, int q2BinIndx = 0, int genIndx=0)
{
